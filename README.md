...
2. The frame is converted into the footage's own encoded color space (auto-detected from the file's color tags — Rec.709, Rec.2020, HLG, PQ, P3 — or overridden manually in the LUT menu), which reconstructs the original code values camera LUTs expect.
3. `CIColorCubeWithColorSpace` applies the 3D LUT, optionally blended with the original by the intensity slider.
4. The handler reads LUT state dynamically, so switching or toggling LUTs never rebuilds the player pipeline — playback keeps rolling, gaplessly.
